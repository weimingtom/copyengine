...
public class FlaDesignFileAnalyser {
	...
	public void analysisFlaDesignFile(FlaFileMeta meta, Element partentElement) {
		 ...
		 */
		Element readRootElement = flaDesignFileDoc.getRootElement().element("timeline").element("DOMTimeline").element("layers");
		for (Iterator layerElementIt = readRootElement.elements("DOMLayer").iterator(); layerElementIt.hasNext();) {
			...
			Element elementsRoot = layerElement.element("frames").element("DOMFrame").element("elements");
			...
			for (Iterator analysElementIt = elementsRoot.elements("DOMSymbolInstance").iterator(); analysElementIt.hasNext();) {
				...
				//make sure the 
				generateChildElementByType(analysElement.attributeValue("name"), generateLayerElement , meta.getFilePathWithoutName()+analysElement.attributeValue("libraryItemName") );
			}
		}
	...
	private void generateChildElementByType(String type, Element parentElement , String possiblePath) {
		StringTokenizer analysTokenizer = new StringTokenizer(type, "-");
		...
		{
			//name ::  xxx_Symbol_xxx
			SymbolElementAnalyser.analyserSymbolNode(null, parentElement, symbolAttribute);
		}
